...
def initialize_database():
    if settings_collection.count_documents({}) == 0:
        settings_collection.insert_one({
            ...
            'wechat': '', 'phone': '', 'address': '福建省南平市浦城县',
...
def index():
    ...
    dogs = list(dogs_collection.find({'status': 'available'}).sort('createdAt', -1))
...
def dog_detail(dog_id):
    ...
    if not dog:
        return redirect(url_for('index'))
    other_dogs = list(dogs_collection.find({'_id': {'$ne': object_id}, 'status': 'available'}).limit(4))
    return render_template('dog_detail.html', settings=settings, dog=dog, other_dogs=other_dogs)
...
def admin_dashboard():
    ...
    for dog in dogs:
        dog['_id'] = str(dog['_id'])
    stats = {'total': dogs_collection.count_documents({}), 'available': dogs_collection.count_documents({'status': 'available'}), 'adopted': dogs_collection.count_documents({'status': 'adopted'})}
...
def admin_add_dog():
    if request.method == 'POST':
        ...
        now = datetime.now()
        dogs_collection.insert_one({'name': request.form.get('name', '').strip(), 'age': request.form.get('age', '').strip(), 'gender': request.form.get('gender', ''), 'breed': request.form.get('breed', '').strip(), 'personality': request.form.get('personality', '').strip(), 'health': request.form.get('health', '').strip(), 'story': request.form.get('story', '').strip(), 'status': request.form.get('status', 'available'), 'photo': photo_base64, 'createdAt': now, 'updatedAt': now})
...
def admin_edit_dog(dog_id):
    ...
    if not dog:
        ...
        return redirect(url_for('admin_dashboard'))
    if request.method == 'POST':
        update_data = {'name': request.form.get('name', '').strip(), 'age': request.form.get('age', '').strip(), 'gender': request.form.get('gender', ''), 'breed': request.form.get('breed', '').strip(), 'personality': request.form.get('personality', '').strip(), 'health': request.form.get('health', '').strip(), 'story': request.form.get('story', '').strip(), 'status': request.form.get('status', 'available'), 'updatedAt': datetime.now()}
        photo_file = request.files.get('photo')
        if photo_file and photo_file.filename:
...
def admin_settings():
    settings = settings_collection.find_one() or {}
    if request.method == 'POST':
        update_data = {'site_name': request.form.get('site_name', '').strip(), 'description': request.form.get('description', '').strip(), 'wechat': request.form.get('wechat', '').strip(), 'phone': request.form.get('phone', '').strip(), 'address': request.form.get('address', '').strip(), 'about': request.form.get('about', '').strip(), 'donation_note': request.form.get('donation_note', '').strip()}
        qr_file = request.files.get('donation_qr')
        if qr_file and qr_file.filename:
            update_data['donation_qr'] = image_to_base64(qr_file)
        if settings:
            settings_collection.update_one({'_id': settings['_id']}, {'$set': update_data})
        else:
