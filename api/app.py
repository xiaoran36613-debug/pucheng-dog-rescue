...
def index():
    ...
    dogs = list(
        dogs_collection.find({'status': 'available'})
        .sort('createdAt', -1)
    )
...
def admin_dashboard():
    ...
    stats = {
        ...
        'available': dogs_collection.count_documents({
            'status': 'available'
        }),
        'adopted': dogs_collection.count_documents({
            'status': 'adopted'
        })
    }
...
@app.route(
    '/admin/dog/delete/<dog_id>',
    methods=['POST']
)
@login_required
def admin_delete_dog(dog_id):
    dogs_collection.delete_one({
        '_id': ObjectId(dog_id)
    })
...
def admin_settings():
    ...
    if request.method == 'POST':
        update_data = {
            ...
            'donation_note': request.form.get(
                'donation_note',
                ''
            ).strip()
        }
