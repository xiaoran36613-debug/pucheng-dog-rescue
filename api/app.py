...
# MongoDB 连接
MONGODB_URI = os.environ.get('MONGODB_URI', 'mongodb://localhost:27017/pucheng_dogs')
...
def init_admin():
    if admin_collection.count_documents({}) == 0:
        if not os.environ.get('ADMIN_PASSWORD'):
            raise RuntimeError('ADMIN_PASSWORD environment variable is required for first-time admin setup')
...
def dog_detail(dog_id):
    ...
    other_dogs = list(dogs_collection.find({
        '_id': {'$ne': ObjectId(dog_id)},
        'status': 'available'
    }).limit(4))
    ...
    if request.method == 'POST':
        ...
        if 'photo' in request.files:
            ...
            if photo_file.filename:
                ...
                photo_base64 = 'data:' + photo_file.content_type + ';base64,' + base64.b64encode(photo_data).decode()
...
def admin_edit_dog(dog_id):
    ...
    if request.method == 'POST':
        ...
        if 'photo' in request.files:
            ...
            if photo_file.filename:
                photo_data = photo_file.read()
                update_data['photo'] = 'data:' + photo_file.content_type + ';base64,' + base64.b64encode(photo_data).decode()

        dogs_collection.update_one({'_id': ObjectId(dog_id)}, {'$set': update_data})
...
def admin_delete_dog(dog_id):
    dogs_collection.delete_one({'_id': ObjectId(dog_id)})
    flash('狗狗已删除', 'success')
    return redirect(url_for('admin_dashboard'))
...
def admin_settings():
    ...
    if request.method == 'POST':
        ...
        if 'donation_qr' in request.files:
            qr_file = request.files['donation_qr']
            if qr_file.filename:
                qr_data = qr_file.read()
                update_data['donation_qr'] = 'data:' + qr_file.content_type + ';base64,' + base64.b64encode(qr_data).decode()
